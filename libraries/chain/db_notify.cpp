...
#include "include/graphene/chain/protocol/types.hpp"
...
struct get_impacted_account_visitor {
    ...
    void operator()(const vindb_block_create_operation &op) {
        _impacted.insert(op.owner);
    }

    void operator()(const invoice_create_operation &op) {
        _impacted.insert(op.contractor);
    }
...
static void get_relevant_accounts(const object *obj, flat_set <account_id_type> &accounts) {
    if (obj->id.space() == protocol_ids) {
        switch ((object_type) obj->id.type()) {
            ...
            case vindb_block_object_type: {
                break;
            }
            case invoice_object_type: {
                const auto &aobj = dynamic_cast<const invoice_object *>(obj);
                assert(aobj != nullptr);
                accounts.insert(aobj->contractor);
                break;
            }
            case invoice_payment_object_type: {
