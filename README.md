...
VinChain Core is the VinChain blockchain implementation and command-line interface.
The web wallet is [VinChain UI](https://github.com/VinChain/VinChain-UI).

**NOTE:** The official VinChain git repository location, default branch, and submodule remotes were recently changed. Existing
repositories can be updated with the following steps:

    git remote set-url origin https://github.com/VinChain/VINchain-blockchain.git
    git checkout master
...
To build after all dependencies are installed:
    ...
    git clone https://github.com/VinChain/VINchain-blockchain.git
    cd VINchain-blockchain
    git checkout <LATEST_RELEASE_TAG>
    git submodule update --init --recursive
    cmake -DCMAKE_BUILD_TYPE=RelWithDebInfo .
    make

**NOTE:** VinChain requires an [OpenSSL](https://www.openssl.org/) version in the 1.0.x series. OpenSSL 1.1.0 and newer are NOT supported. If your system OpenSSL version is newer, then you will need to manually provide an older version of OpenSSL and specify it to CMake using `-DOPENSSL_INCLUDE_DIR`, `-DOPENSSL_SSL_LIBRARY`, and `-DOPENSSL_CRYPTO_LIBRARY`.

**NOTE:** VinChain requires a [Boost](http://www.boost.org/) version in the range [1.57, 1.63]. Versions earlier than
